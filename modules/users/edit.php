...
$data = [
    'pageTile' => 'Update người dùng'
];

//Kiểm tra id cần sửa có tồn tiai5 hay không
$filterAll = filter();
if (!empty($filterAll['id'])) {
    $userId = $filterAll['id'];

    //Kiểm tra $userId có tồn tại trong database không
    //Nếu id tồn tại => thông tin người dùng
    //Nếu id không tồn tại => chuyển hướng về trang list
...
if (isPost()) {
    ...
    if (empty($filterAll['email'])) {
        ...
        $sql = "SELECT id FROM users WHERE email = '$email' AND id <>'$userId' "; // validate email phải khác email edit
        if (getRaws($sql) > 0) {
    ...
    //Kiểm tra nếu người dùng có sửa password
    ...
    //Nếu không có lỗi xảy ra thì tiến hành UPDATE
    if (empty($errors)) {
        ...
        // echo '</pre>';
        // die();
        $condition = "id='$userId'"; //gán biến điều kiện sửa
...
if (!empty($userDetailll)) {
    $old = $userDetailll; // gán biến $old để lấp thông tin vào ô input
}

?>
<div class="container">
    <div class="row" style="margin: 100px auto;">
        <h2 class="text-center text-uppercase">Update ngưởi dùng</h2>
        ...
        <form action="" method="post">
            <div class="row">
                ...
                <div class="col">
                    ...
                    <div class="form-group mg-form">
                        <input name="password" type="text" class="form-control" placeholder="Nhập vào mật khẩu (Không nhập nếu không thay đổi)">
                        <?php
            ...
            <button type="submit" class="mg-btned btn btn-primary btn-block">Update người dùng</button>
